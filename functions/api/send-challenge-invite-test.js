// Test endpoint: sends the challenge invite email to a single address.
...
export async function onRequestPost(context) {
  ...
  if (!testEmail) {
    return new Response(JSON.stringify({ error: "No test_email provided" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  }
  ...
  const res = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      to: [{ email: testEmail }],
      subject: "[TEST] Read the Bible with me this July",
      htmlContent: html,
    }),
  });
  ...
  if (res.ok) {
    ...
    return new Response(JSON.stringify({ error: "Brevo failed", details: err }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
